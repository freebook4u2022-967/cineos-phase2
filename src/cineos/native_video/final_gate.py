...
This module adapts native video pixel evaluators to the provider-neutral
``FirstFilmRunner`` contract. It combines whole-film temporal evidence with
plan-aware scene-boundary evidence and fails closed when measured quality is
rejected. No external generator is used: FFmpeg is only a decoder/sampler.
"""
...
class MeasuredFinalFilmReport:
    """Auditable aggregate of whole-film and edit-boundary pixel evidence."""
...
class MeasuredFinalFilmGate:
    ...
    The gate always runs whole-film temporal sampling. When the shot plan crosses
    scene boundaries it additionally samples decoded frames on both sides of each
    edit and validates those pixels against the authored transition contract.
    """
    ...
    def evaluate(
        ...
        # Preserve order while preventing repeated remediation instructions from
        # multiple boundaries from bloating durable FilmBuild evidence.
