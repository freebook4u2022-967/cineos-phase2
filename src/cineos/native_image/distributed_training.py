...
class DistributedTrainingCoordinator:
    def create_plan(
        ...
        *,
        ...
        if len(eligible) < requested_world_size:
            raise RuntimeError(
                "insufficient eligible GPU workers for distributed training"
            )
        selected = tuple(
            sorted(
                eligible,
                key=lambda item: (item.current_load, -item.vram_gb, item.worker_id),
            )[:requested_world_size]
        )
        return DistributedTrainingPlan(
            selected, requested_world_size, requested_world_size
        )

    def shard_dataset(
        self, sample_count: int, world_size: int
    ) -> tuple[DataShard, ...]:
    ...
    def average_gradients(
        self, gradients: tuple[tuple[float, ...], ...]
    ) -> tuple[float, ...]:
        if not gradients:
        ...
        return tuple(
            sum(vector[index] for vector in gradients) / count for index in range(width)
        )
