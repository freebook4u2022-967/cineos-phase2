...
def _require_bound_shot(record: Mapping[str, Any], *, index: int) -> tuple[str, Path, str]:
    ...
    if len(expected_hash) != 64:
        raise AssemblyError(f"production shot {shot_id} is missing a valid output SHA-256")
    actual_hash = file_hash(output_path)
    if actual_hash != expected_hash:
        raise AssemblyError(f"production shot {shot_id} artifact hash does not match QC evidence")
    ...
    *,
    ...
    if audio_path is not None:
        ...
        if actual_audio_hash != audio_sha256.strip().lower():
            raise AssemblyError("production audio artifact hash does not match supplied evidence")
