...
def test_attested_readiness_rejects_path_replacement_after_hash(tmp_path, monkeypatch):
    runtime = _runtime()
    attestation = _attestation(tmp_path, runtime)
    target = attestation.artifacts[0]
    path = tmp_path / f"{target.key}.json"
    replacement = tmp_path / "post-hash-replacement.json"
    replacement.write_text("post-hash replacement\n", encoding="utf-8")

    real_lstat = production_readiness_module.Path.lstat
    target_lstat_calls = 0

    def replace_before_final_lstat(candidate):
    ...
    monkeypatch.setattr(
        production_readiness_module.Path, "lstat", replace_before_final_lstat
    )
